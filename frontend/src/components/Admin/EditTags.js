...
import { editTags, getATags } from "../../redux/actions/tags";
...
const EditTags = () => {
  ...
  const tag = useSelector((state) => state.tags?.singleTag);

  useEffect(() => {
    getTag();
  }, []);
  const getTag = () => {
    dispatch(getATags(id));
  };
